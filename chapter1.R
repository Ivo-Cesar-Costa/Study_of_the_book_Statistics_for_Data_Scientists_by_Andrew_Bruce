...
state <- read.csv(file.path(PSDS_PATH, 'data', 'state.csv'), sep = ";")
dfw <- read.csv(file.path(PSDS_PATH, 'data', 'dfw_airline.csv'), sep = ";")
sp500_px <- read.csv(file.path(PSDS_PATH, 'data', 'sp500_px.csv'), sep = ";")
sp500_sym <- read.csv(file.path(PSDS_PATH, 'data', 'sp500_sym.csv'), stringsAsFactors = FALSE, sep = ";")
kc_tax <- read.csv(file.path(PSDS_PATH, 'data', 'kc_tax.csv'), sep = ";")
lc_loans <- read.csv(file.path(PSDS_PATH, 'data', 'lc_loans.csv'), sep = ";")
airline_stats <- read.csv(file.path(PSDS_PATH, 'data', 'airline_stats.csv'), stringsAsFactors = FALSE, sep = ";")
...
par(mar=c(4,4,0,1)+.1)
plot(telecom$T, telecom$VZ, xlab="T", ylab="VZ", cex=0.8)
...
x_tab <- CrossTable(lc_loans$grade, lc_loans$status, 
                    prop.c=TRUE, prop.chisq=FALSE, prop.t=FALSE)
